...
// this.currentPage=""
 this.currentRecipe = id;
    ...
    let filtered = this.recipeData.filter(
      recipe => recipe[filter].toLowerCase().includes(input.toLowerCase())
    )
    return filtered
    },
    arrayFilter(input,filter){
      let filtered = this.recipeData.filter(
        recipe => 
        recipe[filter.toLowerCase()].some(i => i.includes(input.toLowerCase()))
        )
